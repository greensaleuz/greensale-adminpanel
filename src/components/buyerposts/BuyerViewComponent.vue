...
export default defineComponent({
  ...
  methods: {
    load() {
      ...
      this.AvarageStar = this.averageStars || 0;
      if (this.status === 0) {
    ...
    exit() {
      localStorage.setItem('buyerrById', this.id?.toString() || '')
    ...
    async stars(stars_number: Number) {
      ...
      var response = await axios.get<PostViewModel>('/api/common/buyer/posts/' + Number(this.id));
