...
export default defineComponent({
  ...
  data() {
    return {
      ...
      list: {} as GetSearchStorageViewModel
    }
  },
  methods: {
    ...
    async getSearch(search: any) {
      this.isLoaded = false;
      var response = await axios.get<GetSearchStorageViewModel>("/api/common/storage/search/info?search=" + search);
      this.isLoaded = true;
  ...
  <ul>
    <div class="cart_wrapper">
      <template v-for="element in postsList" :key="element.id">
        ...
        <StorageViewComponent :id=element.id :UserId=element.userId :fullName=element.fullName
